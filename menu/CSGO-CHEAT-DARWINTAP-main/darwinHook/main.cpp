...
long __stdcall hkEndScene(LPDIRECT3DDEVICE9 o_pDevice)
{
    ...
    if (menu) {
        ...
        if (ttab == 1) {
            ...
            ImGui::Checkbox(xorstr(u8"Триггер бот"), &triggerbot);
            ImGui::Checkbox(xorstr(u8"Прицел"), &awpcrosshair);
            if (awpcrosshair)
            {
                ImGui::SliderInt(xorstr(u8"Размер прицела"), &awpcrosshairWiHe, 1, 10);
            }
        }
        else if (ttab == 2) {
            ...
            ImGui::Checkbox(xorstr(u8"хуета"), &hueta);
            ImGui::Checkbox(xorstr(u8"Изменить FOV"), &fovchanger);
            if (fovchanger) {
                ImGui::SliderInt(xorstr(u8"Значение FOV"), &fov, -200, 200);
            }
            if (ImGui::Button(xorstr(u8"включть все функции"))) {
                espbox = true;
                bunnyhop = true;
                radarhack = true;
                antiflash = true;
